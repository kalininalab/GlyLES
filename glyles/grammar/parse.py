...
class TreeWalker:
    ...
    def parse(self, t: ParseTree, init: str, **kwargs):
        """
        ...
        node_id = self.__add_node(init, **kwargs)

        # add the parsed AST to the networkx graph
        self.__walk(next(t.getChildren()), node_id, **kwargs)

        return self.g

    def __walk(self, t, parent, **kwargs):
        """
        ...
        if len(children) == 2:  # {sac con}
            ...
            node_id = self.__add_node(children[0].symbol.text, **kwargs)
            ...
            parent = self.__walk(children[2], parent, **kwargs)
            node_id = self.__add_node(children[0].symbol.text, **kwargs)
            self.__add_edge(parent, node_id, children[1])
            return node_id
            ...
            self.__walk(children[1], parent, **kwargs)
            return parent
            ...
            node_id = self.__add_node(children[5].symbol.text, **kwargs)
            self.__add_edge(parent, node_id, children[6])
            self.__walk(children[3], node_id, **kwargs)
            node_id2 = self.__add_node(children[0].symbol.text, **kwargs)
...
def parse(iupac, **kwargs):
    """
    Adapter on the Lexer and Parser generated by ANTLR based on Grammar.g4.

    Args:
        iupac (str): IUPAC string of a glycan to be converted to SMILES. The SMILES might currently not end with an
        orientation for the root monomer, so no "Gal(a1-4)Gal a", the a would lead to unspecified behaviour
    ...
    walker = TreeWalker()
    g = walker.parse(tree, init, **kwargs)

    if "output" in kwargs:
        nx.drawing.nx_pydot.write_dot(g, os.path.join(kwargs["output"], f"{iupac}{init}.dot"))
