...
def graph_to_string_int(graph: nx.DiGraph, node2label: callable) -> str:
    ...
    def dfs_to_string(G: nx.DiGraph, idx: int, parent: int) -> str:
        ...
        output = node2label(graph.nodes[idx])
        ...
        for child in children[:-1]:  # iterate over all children except the last one and put them in branching brackets
            output = "[" + dfs_to_string(G, child, idx) + "]" + output
        # put the last child in front of the output, without brackets
        output = dfs_to_string(G, children[-1], idx) + output
        return output

    # get the root node index, assuming the root node has the highest index
    root_idx = [node for node in graph.nodes if graph.in_degree(node) == 0][0]  # max(list(graph.nodes.keys()))
...
def graph_to_string(graph: nx.Graph, node2label: callable) -> str: # IUPAC-condensed glycan string
    ...
    if nx.number_connected_components(graph.to_undirected()) > 1:
        ...
        for p in range(len(parts) - 1):
            ...
            len_org += len(H)
        parts = '}'.join(['{'+graph_to_string_int(p, node2label) for p in parts])
...
class Glycan:
    ...
    def to_iupac(self, slim: bool = False) -> str:
        def node2label(node):
            recipe = node["type"].get_recipe()
            if slim:
                recipe = [(r, s) for r, s in recipe if r not in {"f", "p", "o"}]
            label = "".join([str(r) for r, _ in recipe])
            if slim:
                label = label.replace("L-", "").replace("D-", "")
            return label
        return graph_to_string(self.parse_tree, node2label)
